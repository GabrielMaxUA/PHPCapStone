...
//       // Fetch data from the main_gallery table
//       $query = "SELECT archLow, archHigh FROM architecture_gallery WHERE pictureID = ?";
//       $stmt = $con->prepare($query);
//       $stmt->bind_param('i', $pictureID);
//       $stmt->execute();
//       $result = $stmt->get_result();
...
//       $row = $result->fetch_assoc();
//       $filePathLow =  $row['archLow'];
//       $filePathHigh = $row['archHigh'];

//       // Delete from the database
//       $deleteQuery = "DELETE FROM architecture_gallery WHERE pictureID = ? LIMIT 1";
//       $stmt = $con->prepare($deleteQuery);
//       $stmt->bind_param('i', $pictureID);
